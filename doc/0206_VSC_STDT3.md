<h3 align='right'><span style="text-decoration:none;"><a href="./0001_TOC.md" title="Table Of Content">TOC</a></span></h3>

<h1 align='center'>VSC - STDT 3</h1>

<h3 align="center">
  <a href="./0205_VSC_STDT2.md">← 0205_VSC_STDT2</a>
                     
  <a href="./0300_GIT_DEEP.md">0300_GIT_DEEP →</a>
</h3>

---

## 👉 [La vidéo du cycle complet : De la recheche d'un truc à faire jusqu'à sa validation officielle](https://youtu.be/oRuxVoGKjOs) - OU : Comment confirmer sa place dans la Légende, les mains dans les poches

→ Situation / Cas de figure :

Tu n'es même pas chez toi, donc pas sur ton Matos chéri habituel, et tu as quand-même un PC (ou autre) connecté sous le coude, et quelques minutes de dispo... Et surtout, en plus, fondamentalement, un souhait absolu d'exister, de te rendre utile pour la Communauté...

... Alors, regarde cette vidéo (Rappel: ***MP21170*** n'est qu'un *fake user*, juste pour nous et nos démos dans la doc, toi partout à la place, ça doit afficher ton pseudo GH)

Si t'es largué à un moment, look dans la description ! Et sinon... [Plan ORSEC... Heu, non, juste HELP](https://github.com/GrCOTE7/gsm/blob/main/doc/0103_GIT_USE.md#avant-daller--loin-teste-que-lapp-marche-au-moins-pour-toi-l%C3%A0-en-local-et-sinon-chat-issue-bref-plan-orsec--heu-simplement-page-daide-plut%C3%B4t-) 😉 !

---

### 👉 Tu saurais reproduire tout ce que tu as vu dans la Vidéo ci-dessus...? Alors, tu mérites pleinement d'avoir ton Niveau 🎖️ mis à jour dans le **[Tableau d'Honneur](./7777_HONOR.md) ✌️**

### → Bien que tu avais le droit de PR tes niveaux intermédiaires après le 0200, tu peux y mettre maintenant le niveau **0300** 🎉 en face de ton pseudo/nom - Fais bien-sûr la PR pour que cela soit complètement officiel... Et dans la Légende 💪 !

---

<h3 align="center">
  <a href="./0205_VSC_STDT2.md">← 0205_VSC_STDT2</a>
                     
  <a href="./0300_GIT_DEEP.md">0300_GIT_DEEP →</a>
</h3>
