# Gestion de l’environnement (.env, variables, PATH, uv) 

# Read-DotEnv → lit un fichier .env et exporte les variables dans Env:
# Set-DotEnvValue → modifie ou ajoute une clé dans .env
# Get-EnvInt → lit une variable d’environnement et la convertit en entier
# Initialize-UvEnvironment → détecte uv.exe dans plusieurs chemins possibles
...
# Gestion des fenêtres Windows Terminal / Console
...
# ----------------------------------------------------------------------
#  Détection du mode d’exécution : MODES SPÉCIAUX (commentés pour l'instant)
# ----------------------------------------------------------------------
