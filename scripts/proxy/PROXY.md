...
Installer un starter auto (Windows)

1. WIN + R → shell:startup

2. **Avec CLI** : Clic droit > Nouveau > Raccourci, puis mettre la cible :

   ```bash
   node "C:\gsm\proxy.js"
   ```

   > ⚠️ Cette commande lance le proxy **en avant-plan** dans une fenêtre console qui
   > reste ouverte tant que le serveur tourne.

3. **Sans CLI (recommandé)** : Clic droit > Nouveau > Document texte, mettre ce code
   dedans, puis renommer le fichier en `proxy.bat` :

   ```bat
   @echo off
   rem Démarre le proxy - Console minimisée.
...
1. > Démarrage manuel : le proxy écoute sur `http://localhost:3000`.
...
4. > Injection du 'Thinking' : sur chaque `POST /v1/chat/completions`, le payload
...
5. > Streaming : si la requête demande `stream: true`, le proxy ajoute
...
6. > Erreurs : les réponses d'erreur venant de DeepSeek sont relues (le corps est un
   stream) puis retransmises proprement en JSON avec le bon status HTTP.

7. > Source de vérité des modèles : les modèles et leurs prix sont chargés depuis
...
8. > Config Cline associée (provider « OpenAI Compatible ») :
